...
class TestGeneration(tf.test.TestCase):
    ...
    def testGenerateSimple(self):
        ...
        proba = self.net.predict_proba(waveform, IDs)

        with self.test_session() as sess:
            sess.run(tf.initialize_all_variables())
            proba = sess.run(proba, feed_dict={waveform: data, IDs: ID})
    ...
    def testGenerateFast(self):
        ...
        proba = self.net.predict_proba_incremental(waveform, IDs)
    ...
    def testCompareSimpleFast(self):
        ...
        proba = self.net.predict_proba(waveform, IDs)
        proba_fast = self.net.predict_proba_incremental(waveform, IDs)
        with self.test_session() as sess:
            sess.run(tf.initialize_all_variables())
            sess.run(self.net.init_ops)
            ...
            for x in data[:-1]:
                proba_fast_ = sess.run(
                    ...
                    feed_dict={waveform: x, IDs: ID})

            # Get the last sample from the incremental generator
            proba_fast_ = sess.run(
                proba_fast,
                feed_dict={waveform: data[-1], IDs: ID})
            # Get the sample from the simple generator
            proba_ = sess.run(proba, feed_dict={waveform: data, IDs: ID})
