...
def list_mean(A,i):#前n项的和作为此项，但数组大小不减小
    sum=0.0
    smooth_int=5
    if i<smooth_int:
        print "error"
    else:
        for j in range(smooth_int):
            sum+=A[i-j]
    return sum/smooth_int



#平滑曲线
for i in xrange(5,len(list_train_loss)):
    list_train_loss[i]=list_mean(list_train_loss,i)
for i in xrange(5,len(list_train_tl_L1)):
    list_train_loss[i]=list_mean(list_train_tl_L1,i)
for i in xrange(5,len(list_val_loss)):
    list_train_loss[i]=list_mean(list_val_loss,i)
for i in xrange(5,len(list_val_tl_L1)):
...
savefig('./my3'+'smooth'+filename+'.jpg')
...
savefig('./my4'+'smooth'+filename+'.jpg')
print "ok"
